...
export default function DatePickerDialog(props: ISimpleDatePickerProps) {
  ...
  const prevMonthButtonClassName = [
    'simple-date-picker__prev-month',
    !hasPrevMonth ? 'simple-date-picker__prev-month--disabled' : ''
  ].join(' ');

  const nextMonthButtonClassName = [
    'simple-date-picker__next-month',
    !hasNextMonth ? 'simple-date-picker__next-month--disabled' : ''
  ].join(' ');
