...
const DatePickerDialogCalendar = (props: IProps) => {
  ...
  const getBodyCellClass = (day: Moment) => {
    ...
    const isToday = !empty && day.isSame(today, 'day');
    const isSelected = !empty && selectedDate && day.isSame(selectedDate, 'day');
    const isNotAllowed = isMinOrMax(day);
    return [
      'simple-date-picker-calendar__body-cell',
      empty ? 'simple-date-picker-calendar__body-cell--empty' : '',
      isToday ? 'simple-date-picker-calendar__body-cell--today' : '',
      isSelected ? 'simple-date-picker-calendar__body-cell--selected' : '',
      isNotAllowed ? 'simple-date-picker-calendar__body-cell--not-allowed' : ''
    ].join(' ');
  };
